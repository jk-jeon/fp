...
#ifndef JKJ_HEADER_FP_BIGINT
#define JKJ_HEADER_FP_BIGINT
...
namespace jkj::fp {
	namespace detail {
		...
		struct bigint_impl : public bigint_base {
			...
			friend constexpr std::size_t log2p1(bigint_impl const& n) noexcept {
				return n.leading_one_pos.element_pos * element_number_of_bits
					+ n.leading_one_pos.bit_pos;
			}

			bool is_zero() const noexcept {
				return leading_one_pos.bit_pos == 0;
			}

			static bigint_impl power_of_5(std::size_t exp) noexcept {
				bigint_impl ret{ 1 };
		...
		public:
			...
			constexpr bigint_impl& operator++()& {
				std::size_t idx = 0;
				while (elements[idx] == element_type(-1)) {
					elements[idx] = 0;
					++idx;
			...
			friend bigint_impl operator*(element_type x, bigint_impl const& y) {
				auto r = y;
				return r *= x;
			}
			...
			bigint_impl& operator*=(bigint_impl const& n)& {
				auto result = *this * n;
				*this = result;
				return *this;
			}
			...
			// Left shift (note: this shift disallows overflow)
			bigint_impl& operator<<=(std::size_t sh) & {
				if (is_zero() || sh == 0) {
					return *this;
				}

				auto current_leading_one_pos = log2p1(*this);

				// Overflow check
				assert(current_leading_one_pos + sh <= element_number_of_bits * array_size);

				leading_one_pos_t new_leading_one_pos;
				new_leading_one_pos.bit_pos =
					((current_leading_one_pos + sh - 1) % element_number_of_bits) + 1;
				new_leading_one_pos.element_pos =
					((current_leading_one_pos + sh - 1) / element_number_of_bits);

				auto element_move_distance = sh / element_number_of_bits;
				auto bit_move_distance = sh % element_number_of_bits;

				// Do shift
				if (bit_move_distance == 0) {
					assert(element_move_distance != 0);
					for (std::size_t idx = new_leading_one_pos.element_pos;
						idx >= element_move_distance; --idx)
					{
						elements[idx] = elements[idx - element_move_distance];
					}
					for (std::size_t idx = 0; idx < element_move_distance; ++idx) {
						elements[idx] = 0;
					}
				}
				else {
					if (leading_one_pos.element_pos + element_move_distance + 1 < array_size) {
						elements[leading_one_pos.element_pos + element_move_distance + 1] =
							(elements[leading_one_pos.element_pos]
								>> (element_number_of_bits - bit_move_distance));
					}
					assert(leading_one_pos.element_pos + element_move_distance < array_size);
					for (std::size_t idx = leading_one_pos.element_pos + element_move_distance;
						idx > element_move_distance; --idx)
					{
						elements[idx] = (elements[idx - element_move_distance] << bit_move_distance)
							| (elements[idx - element_move_distance - 1] >>
								(element_number_of_bits - bit_move_distance));
					}
					elements[element_move_distance] = (elements[0] << bit_move_distance);
				...
				leading_one_pos = new_leading_one_pos;
				return *this;
			}

			bigint_impl operator<<(std::size_t sh) const {
				auto ret = *this;
				ret <<= sh;
				return ret;
			}

			// Right shift (note: this shift is well-defined even for too large shift)
			bigint_impl& operator>>=(std::size_t sh) & {
				if (is_zero() || sh == 0) {
					return *this;
				}
				...
				if (bit_move_distance == 0) {
					assert(element_move_distance != 0);
					for (std::size_t idx = 0; idx <= new_leading_one_pos.element_pos; ++idx) {
						elements[idx] = elements[idx + element_move_distance];
					}
					for (std::size_t idx = new_leading_one_pos.element_pos + 1;
						idx <= leading_one_pos.element_pos; ++idx)
					{
						elements[idx] = 0;
					}
				}
				else {
					assert(element_move_distance <= leading_one_pos.element_pos);
					for (std::size_t idx = 0;
						idx + element_move_distance < leading_one_pos.element_pos; ++idx)
					{
						elements[idx] = (elements[idx + element_move_distance] >> bit_move_distance)
							| (elements[idx + element_move_distance + 1] <<
								(element_number_of_bits - bit_move_distance));
					}
					elements[leading_one_pos.element_pos - element_move_distance]
						= (elements[leading_one_pos.element_pos] >> bit_move_distance);

					for (std::size_t idx = leading_one_pos.element_pos - element_move_distance + 1;
						idx <= leading_one_pos.element_pos; ++idx)
					{
						elements[idx] = 0;
					}
			...
			friend bigint_impl lower_bits_of(bigint_impl const& n, std::size_t bits) {
				if (bits >= log2p1(n)) {
					return n;
				}

				auto number_of_preserved_elements = bits / bigint_impl::element_number_of_bits;
				auto remaining_bits = bits % element_number_of_bits;

				bigint_impl ret;
				std::copy(std::begin(n.elements), std::end(n.elements), std::begin(ret.elements));
				
				for (std::size_t idx = number_of_preserved_elements + 1;
					idx <= n.leading_one_pos.element_pos; ++idx)
				{
					ret.elements[idx] = 0;
				}
				ret.elements[number_of_preserved_elements]
					= (((element_type(1) << remaining_bits) - 1) & n.elements[number_of_preserved_elements]);

				// Find the leading one position
				std::size_t idx = number_of_preserved_elements;
				for (; idx > 0; --idx) {
					if (ret.elements[idx] != 0) {
						break;
					}
				}
				ret.leading_one_pos.element_pos = idx;
				ret.leading_one_pos.bit_pos = log2p1(ret.elements[idx]);
				return ret;
			}

			// Iteratively divide by 2 until *this becomes an odd number
			// Returns how many times the division was performed
			// Precondition: *this must not be zero
			std::size_t factor_out_2() & {
				assert(leading_one_pos.bit_pos != 0);

				std::size_t factor_count = 0;
				for (std::size_t idx = 0; idx <= leading_one_pos.element_pos; ++idx) {
					if (elements[idx] == 0) {
						factor_count += element_number_of_bits;
					}
					else {
						std::size_t factor_bit_count = 0;
						for (std::size_t c = element_number_of_bits - 1; c > 0; --c) {
							if (((elements[idx] >> c) << c) == elements[idx]) {
								factor_bit_count = c;
								break;
							}
						}
						assert(factor_bit_count < element_number_of_bits);
						factor_count += factor_bit_count;
						
						// Shift elements
						if (factor_bit_count == 0) {
							for (std::size_t from_idx = idx;
								from_idx <= leading_one_pos.element_pos; ++from_idx)
							{
								elements[from_idx - idx] = elements[from_idx];
							}
						}
						else {
							for (std::size_t from_idx = idx;
								from_idx < leading_one_pos.element_pos; ++from_idx)
							{
								elements[from_idx - idx] =
									(elements[from_idx] >> factor_bit_count) |
									(elements[from_idx + 1] << (element_number_of_bits - factor_bit_count));
							}
							elements[leading_one_pos.element_pos - idx] =
								elements[leading_one_pos.element_pos] >> factor_bit_count;
						}
						for (std::size_t to_idx = leading_one_pos.element_pos - idx + 1;
							to_idx <= leading_one_pos.element_pos; ++to_idx)
						{
							elements[to_idx] = 0;
						}

						leading_one_pos.element_pos -= idx;
						if (leading_one_pos.bit_pos > factor_bit_count) {
							leading_one_pos.bit_pos -= factor_bit_count;
						}
						else {
							--leading_one_pos.element_pos;
							leading_one_pos.bit_pos =
								leading_one_pos.bit_pos + element_number_of_bits - factor_bit_count;
						}
						break;
					}
				}
			...
			// Count how many 2's can be factored out
			std::size_t count_factor_of_2() const {
				assert(leading_one_pos.bit_pos != 0);
