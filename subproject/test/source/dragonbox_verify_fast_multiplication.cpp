...
{
	using impl = jkj::fp::detail::dragonbox::impl<Float>;
	...
	using jkj::fp::detail::log::floor_log10_pow2_minus_log10_4_over_3;
	using jkj::fp::detail::log::floor_log2_pow10;
	...
	for (int e = impl::min_exponent + 1; e <= impl::max_exponent; ++e) {
		...
		// Load cache
		auto const cache = jkj::fp::policy::cache::fast.get_cache<impl::format>(-minus_k);
...
static bool verify_fast_multiplication_yru()
{
	...
	for (int k = impl::min_k; k < 0; ++k) {
		auto const cache = jkj::fp::policy::cache::fast.get_cache<impl::format>(k);

		// Since p + beta <= q, suffices to check that the lower half of the cache is not 0
		auto const lower_half = [cache] {
			if constexpr (impl::format == jkj::fp::ieee754_format::binary32)
			{
