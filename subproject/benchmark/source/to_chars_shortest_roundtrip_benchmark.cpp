		...
		std::size_t number_of_digits_samples_per_digits)
	{
		...
		for (unsigned int digits = 1; digits <= max_digits; ++digits) {
			...
			for (auto& sample : samples_[digits])
				sample = jkj::fp::detail::randomly_generate_float_with_given_digits<Float>(digits, rg_);
		}
	}
...
register_function_for_to_chars_shortest_roundtrip_benchmark::register_function_for_to_chars_shortest_roundtrip_benchmark(
	...
	std::size_t number_of_iterations)
{
	...
	char buffer[64];
	typename jkj::fp::ieee754_traits<Float>::carrier_uint br;
	for (auto& name_result_pair : out) {
		for (auto const& data_time_pair : name_result_pair.second[0]) {
			...
			jkj::fp::to_chars_shortest_scientific(data_time_pair.first, buffer);
			out_file << "\"" << name_result_pair.first << "\"," << buffer << "," <<
				"0x" << std::hex << std::setfill('0');
			if constexpr (jkj::fp::ieee754_traits<Float>::format == jkj::fp::ieee754_format::binary32)
			{
				out_file << std::setw(8);
			}
			else
...
int main() {
	...
	if constexpr (benchmark_float) {
		std::cout << "[Running shortest-roundtrip formatting benchmark for binary32...]\n";
	...
	if constexpr (benchmark_double) {
		std::cout << "[Running shortest-roundtrip formatting benchmark for binary64...]\n";
